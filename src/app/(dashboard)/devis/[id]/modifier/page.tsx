...
export default function ModifierDevisPage() {
    ...
    fetch(`/api/devis/${devisId}`)
      ...
      .then((data) => {
        if (data.statut !== "BROUILLON") {
          toast.error("Seuls les devis brouillon peuvent être modifiés");
        ...
        setDateValidite(
          data.dateValidite
            ? new Date(data.dateValidite).toISOString().split("T")[0]
            : ""
        );
        setLignes(
          data.lignes.map((l: Ligne) => ({
  ...
  const totalTVA = lignes.reduce(
    (s, l) => s + l.quantite * l.prixUnitaire * (l.tauxTVA / 100),
    0
  );
  ...
  if (loading)
    return <div className="text-[var(--text-muted)]">Chargement...</div>;
                ...
                >
                  <div className="flex gap-2 flex-wrap items-end">
                    ...
                    <div className="w-28 space-y-1">
                      ...
                      <span className="flex h-10 items-center font-medium gradient-text">
                        {formatCurrency(l.quantite * (l.prixUnitaire || 0))}
                      </span>
                    </div>
                    <Button
                      type="button"
