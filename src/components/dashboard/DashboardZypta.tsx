    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-zypta bg-gradient-to-br from-zypta-blue/10 via-supernova-light/30 to-zypta-orange/10 p-6 shadow-zypta">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2 rounded-xl bg-white/80 px-4 py-2 shadow-sm">
            <Sun className="h-5 w-5 text-zypta-orange" />
            <span className="text-sm font-medium">Ensoleillé, 18°C</span>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="group rounded-zypta border border-slate-200/80 bg-white p-5 shadow-zypta transition-all duration-300 hover:shadow-zypta-lg hover:-translate-y-0.5">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-slate-500">CA du mois</span>
            <Euro className="h-5 w-5 text-zypta-blue opacity-80" />
          </div>
          <p className="mt-2 text-2xl font-bold text-slate-900">{formatCurrency(caMois)}</p>
        ...
        <div className="group rounded-zypta border border-slate-200/80 bg-white p-5 shadow-zypta transition-all duration-300 hover:shadow-zypta-lg hover:-translate-y-0.5">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-slate-500">Devis en attente</span>
            <FileText className="h-5 w-5 text-zypta-blue opacity-80" />
          </div>
          <p className="mt-2 text-2xl font-bold text-slate-900">{devisEnAttente}</p>
          <Link href="/devis" className="mt-1 text-sm font-medium text-zypta-blue hover:underline">
            Voir les devis →
          </Link>
        </div>
        ...
        <div className="group rounded-zypta border border-slate-200/80 bg-white p-5 shadow-zypta transition-all duration-300 hover:shadow-zypta-lg hover:-translate-y-0.5">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-slate-500">Factures impayées</span>
            <Receipt className="h-5 w-5 text-zypta-orange opacity-80" />
          </div>
          <p className="mt-2 text-2xl font-bold text-slate-900">{facturesImpayees}</p>
          {facturesRetard30 > 0 && (
        ...
        <div className="group rounded-zypta border border-slate-200/80 bg-white p-5 shadow-zypta transition-all duration-300 hover:shadow-zypta-lg hover:-translate-y-0.5">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-slate-500">Taux conversion</span>
            <TrendingUp className="h-5 w-5 text-green-500 opacity-80" />
          </div>
          <p className="mt-2 text-2xl font-bold text-slate-900">{tauxConversion.toFixed(1)}%</p>
          <p className="mt-1 text-sm text-slate-500">Devis → Chantier</p>
        </div>
      </div>
      ...
      {chantiers.length > 0 && (
        <div className="rounded-zypta border border-slate-200/80 bg-white p-6 shadow-zypta">
          <h2 className="text-lg font-bold text-slate-900">Chantiers en cours</h2>
          <p className="text-sm text-slate-500">Frise des chantiers avec avancement</p>
          <div className="mt-4 flex gap-4 overflow-x-auto pb-2">
            {chantiers.map((c) => (
              <div
                key={c.id}
                className="min-w-[200px] flex-shrink-0 rounded-xl border border-slate-200/80 p-4 transition-all hover:shadow-md"
              >
                  ...
                  <div
                    className={`h-full rounded-full ${
                      c.statut === "TERMINE"
                        ...
                        : c.statut === "EN_COURS"
                        ? "bg-zypta-blue"
                        : "bg-zypta-orange animate-pulse-soft"
                    }`}
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="rounded-zypta border border-slate-200/80 bg-white p-6 shadow-zypta">
          <h3 className="flex items-center gap-2 font-bold text-slate-900">
            <AlertTriangle className="h-5 w-5 text-zypta-orange" />
            Devis à relancer
          </h3>
            ...
            {devisARelancer.length === 0 ? (
              ...
              devisARelancer.map((d) => (
                <Link
                  key={d.id}
                  href={`/devis/${d.id}`}
                  className="block rounded-lg border border-slate-100 p-3 transition-all hover:border-zypta-blue/30 hover:bg-zypta-blue/5"
                >
        ...
        <div className="rounded-zypta border border-slate-200/80 bg-white p-6 shadow-zypta">
          <h3 className="flex items-center gap-2 font-bold text-slate-900">
            <Send className="h-5 w-5 text-zypta-blue" />
            Factures à envoyer
          </h3>
          <div className="mt-4 space-y-3">
      ...
      <div className="rounded-zypta border border-slate-200/80 bg-white p-6 shadow-zypta">
        <h3 className="font-bold text-slate-900">Cette semaine</h3>
        <div className="mt-4 flex gap-2">
          {["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((j, i) => (
            <div
              key={j}
              className="flex flex-1 flex-col items-center rounded-lg border border-slate-100 p-2"
            >
              <span className="text-xs text-slate-500">{j}</span>
              <span className="mt-1 text-lg font-bold text-slate-900">{i + 18}</span>
              <div className="mt-2 flex gap-1">
                {i === 2 && <span className="h-2 w-2 rounded-full bg-zypta-blue" />}
                {i === 4 && <span className="h-2 w-2 rounded-full bg-zypta-orange" />}
              </div>
            </div>
          ))}
        </div>
