...
export function DashboardZypta({
  ...
  return (
    ...
    >
      ...
      <motion.div variants={item} className="dashboard-card glass relative overflow-hidden rounded-zypta-lg border border-[var(--border)] p-6 shadow-zypta ring-1 ring-supernova/20 transition-shadow hover:shadow-glow-supernova">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2 rounded-xl bg-[var(--bg-card)] px-4 py-2 shadow-sm ring-1 ring-supernova/20">
      ...
      <motion.div variants={item} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="dashboard-card glass group rounded-zypta border border-[var(--border)] p-5 shadow-zypta transition-all duration-200 hover:-translate-y-0.5 hover:shadow-glow-supernova hover:ring-1 hover:ring-supernova/30">
      ...
      {/* Timeline chantiers */}
      {chantiers.length > 0 && (
        <motion.div variants={item} className="dashboard-card glass rounded-zypta border border-[var(--border)] p-6 shadow-zypta ring-1 ring-supernova/10 transition-shadow hover:shadow-glow-violet">
          ...
          <div className="mt-4 flex gap-4 overflow-x-auto pb-2">
            {chantiers.map((c) => (
              <div
                key={c.id}
                className="min-w-[200px] flex-shrink-0 rounded-xl border border-[var(--border)] p-4 transition-all hover:ring-1 hover:ring-supernova/30"
              >
      ...
      <motion.div variants={item} className="grid gap-6 md:grid-cols-3">
        <div className="dashboard-card glass rounded-zypta border border-[var(--border)] p-6 shadow-zypta ring-1 ring-supernova/10 transition-shadow hover:shadow-glow-violet">
            ...
            {devisARelancer.length === 0 ? (
              ...
              devisARelancer.map((d) => (
                <Link
                  key={d.id}
                  href={`/devis/${d.id}`}
                  className="block rounded-lg border border-[var(--border)] p-3 transition-all hover:ring-1 hover:ring-supernova/30"
                >
        ...
        <div className="dashboard-card glass rounded-zypta border border-[var(--border)] p-6 shadow-zypta ring-1 ring-red-500/20 transition-shadow hover:shadow-glow-red">
            ...
            {facturesEnRetard.length === 0 ? (
              ...
              facturesEnRetard.map((f) => (
                <Link
                  key={f.id}
                  href={`/factures/${f.id}`}
                  className={`block rounded-lg border p-3 transition-all hover:ring-1 hover:ring-red-400/40 ${
